...
In this analysis, I will conducted DA taxa analysis on AZM and Placebo samples  at Week 72 similar to week 48 analysis
...
```{r, Library sizes}
LibSizes<-sample_sums(t18g)
hist(LibSizes, xlab="Libary Size", main="Library sizes of AZM and Placebo samples from 72-week visit")
```
...
The library sizes vary greately from 3677 to 61260. Solution to varying library sizes is normalisation. Methods include TSS, SCC, VST, RLE, TMM, CLR, ALR. So for the Wilcoxon test that I will use, I will use TSS, CLR and VST normalisations.
...
0.5% prevalence filtering reduced taxa from 444 to 154.
...
t18fg.TSS<-transform_sample_counts(t18fg, function(x) { x/sum(x)})
OTU.TSSfg<-otu_table(t18fg.TSS)
group<-meta(t18fg)$trial_arm
...
t18fg.addcte<-transform_sample_counts(t18fg, function(x) {x+1})#this is important else you will get the error "Error in estimateSizeFactorsForMatrix(counts(object), locfunc = locfunc,  : every gene contains at least one zero, cannot compute log geometric means""

df<-phyloseq_to_deseq2(t18fg.addcte, ~ trial_arm)
...
OTU.clrfg<-otu_table(t18fg_CLR)

group<-meta(t18fg_CLR)$trial_arm
...
#DESeq2 with apeglm
```{r}
dds <- phyloseq_to_deseq2(t18fg, ~ trial_arm)      #convert to DESeq2 and DGEList objects
dds
...
Deseq2 here detected  DA taxa-Lautropia
...
```{r}
ancom_da <- ancombc(phyloseq = t18fg, formula = "trial_arm", 
...
```{r}
#Run ALDEx2
aldex2_da <- ALDEx2::aldex(data.frame(phyloseq::otu_table(t18fg)), phyloseq::sample_data(t18fg)$trial_arm, test="t", effect = TRUE, denom="iqlr")
```
...
taxa_info <- data.frame(tax_table(t18fg))
...
```{r}
mas1 <- Maaslin2(
  input_data = data.frame(otu_table(t18fg)),
  input_metadata = data.frame(sample_data(t18fg)),
...
data already CLR transformed hence normalisation option selected is "NONE"

```{r}
mas3 <- Maaslin2(
  input_data = data.frame(otu_table(t18fg_CLR)),
  input_metadata = data.frame(sample_data(t18fg_CLR)),
