---
title: "LME analysis of 16S load"
...
#Using data where baseline data has been repeated respectively throughout all visits- "data_for_regression20thDec2021" use this for those variables not working above becuase only available for baseline
...
```{r}
#----------Log10 transformation of 16S rRNA copies-----------------
...
lmer_16s <-lapply(c("trial_arm","visit","adherence","site", "agey", "sex","colmonthgp_mal2" ,"f24mrcscore_baseline ","fev1z","fvc", "fvcz", "fev1fvcz","fvcpcpred", "fevpcpred","zbmiuk","zwauk", "zhauk", "cd4enrl_baseline","cd4", "vload", "normalwaz_reg","normalhaz_reg","acut_exac12m", "hospitalised12m" , " antibiotic12m" , "any3_reasons1",   "f11cotri_baseline","agegpenrl_baseline","duryartgp_baseline", "abnrr", "abnsat", "abnhr","vlenrl_baseline","vlsup_reg","artdrug1stline_baseline","f14adm_baseline",  "f16tbtreat_baseline", "ageyart_baseline", "duryart_baseline", " cd4gp_baseline", "agegpsym_baseline", "mrcgr1_baseline") ,
                 ...
                 function(var) {
                   ...
                   formula    <- as.formula(paste("templatedna_16sqpcr_copies ~ trial_arm*visit + (1|studyno)+", var))
...
lapply(lmer_16s, function(x) write.table( data.frame(x), 'results/lmer_results_16Scopies_21stDec2021AZM_as_reference1.csv'  , append= T, sep=',' ))
```
...
lapply(lmer_16s, function(x) write.table( data.frame(x), 'results/lmer_results_16Scopies_21stDec2021placebo_as_reference1.csv'  , append= T, sep=',' ))
```
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit+
                                             +f24mrcscore_baseline+  (1|studyno), data=Trial1)

summary(rm3.templatedna_16sqpcr_copies.all)
anova(rm3.templatedna_16sqpcr_copies.all)
print("Results for table")
fixef(rm3.templatedna_16sqpcr_copies.all)
confint(rm3.templatedna_16sqpcr_copies.all, level = 0.95)
```
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit
                                             +agegpenrl_baseline+  (1|studyno), data=Trial1)
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit
                                             +duryartgp_baseline+  (1|studyno), data=Trial1)
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit*adherence+
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit*acut_exac12m+
                                           (1|studyno), data=Trial1)
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit*hospitalised12m+
                                           (1|studyno), data=Trial1)
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit*antibiotic12m+
                                           (1|studyno), data=Trial1)
...
```{r}
rm3.templatedna_16sqpcr_copies.all = lmer(templatedna_16sqpcr_copies ~ trial_arm*visit*any3_reasons1+
                                           (1|studyno), data=Trial1)
...
```{r}
#AZM as reference
 md2 <- lmer(templatedna_16sqpcr_copies ~ trial_arm+trial_arm*visit+ 
             +adherence+site +agegpenrl_baseline +sex +colmonthgp_mal2 +f24mrcscore_baseline+fev1z+ normalhaz_reg+
               f11cotri_baseline+duryartgp_baseline+f14adm_baseline+vlsup_reg+
               f16tbtreat_baseline+ (1|studyno),
              data = Trial)
                   summary(md2)
                   confint(md2, level = 0.95)
                   anova(md2)#obtain p values for multi-level factors/variables
                   ...
                   table3  
  write.csv(table3, "results/16Scopies_multivariate21122021AZM_as_ref.csv")  
```
 ...
 md2 <- lmer(templatedna_16sqpcr_copies ~ trial_arm+trial_arm*visit+ 
             +adherence+site +agegpenrl_baseline +sex +colmonthgp_mal2 +f24mrcscore_baseline+fev1z+ normalhaz_reg+
              ...
              data = Trial1)
                   ...
                   table3 <- as.data.frame(cbind(beta, CI, pvalue))# Bind columns together as dataset
                   table3      
write.csv(table3, "results/16Scopies_multivariate21122021Placebo_as_ref.csv")  
```
