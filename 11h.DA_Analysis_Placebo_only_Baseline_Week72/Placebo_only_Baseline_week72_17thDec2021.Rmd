---
title: "Differential abundance analysis of Placebo samples at baseline and week 72 using
  nine tools"
output:
  html_document:
    df_print: paged
...
In this analysis, I will conducted DA taxa analysis on Placebo Baseline and Week 72 samples  
...
Placebo018pfg.TSS<-transform_sample_counts(Placebo018pfg, function(x) { x/sum(x)})
OTU.TSSfg<-otu_table(Placebo018pfg.TSS)
group<-meta(Placebo018pfg)$visit
...
Placebo018pfg.addcte<-transform_sample_counts(Placebo018pfg, function(x) {x+1})#this is important else you will get the error "Error in estimateSizeFactorsForMatrix(counts(object), locfunc = locfunc,  : every gene contains at least one zero, cannot compute log geometric means""

df<-phyloseq_to_deseq2(Placebo018pfg.addcte, ~ visit)
...
OTU.clrfg<-otu_table(Placebo018pfg_CLR)

group<-meta(Placebo018pfg_CLR)$visit
...
```{r}
dds <- phyloseq_to_deseq2(Placebo018pfg, ~ visit)      #convert to DESeq2 and DGEList objects
dds
...
```{r}
ancom_da <- ancombc(phyloseq = Placebo018pfg, formula = "visit", 
...
```{r}
#Run ALDEx2
aldex2_da <- ALDEx2::aldex(data.frame(phyloseq::otu_table(Placebo018pfg)), phyloseq::sample_data(Placebo018pfg)$visit, test="t", effect = TRUE, denom="iqlr")
```
...
taxa_info <- data.frame(tax_table(Placebo018pfg))
...
```{r}
mas1 <- Maaslin2(
  input_data = data.frame(otu_table(Placebo018pfg)),
  input_metadata = data.frame(sample_data(Placebo018pfg)),
  output = "./MaAsLin2_Placebo018_random_effectsTSS_LOG",
  min_abundance = 0.0,
  min_prevalence = 0.0,
  normalization = "TSS",
  transform = "LOG",
...
```{r}
fdr_mas1
write.csv(fdr_mas1, "MaAsLin2_Placebo018_random_effectsTSS_LOG/fdr_mas1No_random_effects.csv")
```
...
```{r}
mas3 <- Maaslin2(
  input_data = data.frame(otu_table(Placebo018pfg_CLR)),
  input_metadata = data.frame(sample_data(Placebo018pfg_CLR)),
  output = "./MaAsLin2_Placebo018_random_effectsCLRYao_NONE",
  min_abundance = 0.0,
  min_prevalence = 0.0,
  normalization = "NONE",
  transform = "NONE",
...
fdr_mas3
write.csv(fdr_mas3, "./MaAsLin2_Placebo018_random_effectsCLRYao_NONE/fdr_mas3_NONE_None_No_random_effectsYAOCLR.csv")
```
...
otu_data = as.matrix(Placebo018pfg@otu_table)
otu_data<-as.data.frame(otu_data)

#Preparing meta_data  for Data preprocessing step
meta_data<-as.matrix(sample_data(Placebo018pfg))
meta_data<-as.data.frame(meta_data)
write.csv(meta_data, "res/meta_data_paired_Placebo072.csv")
#edit im excel so the sample ids have a column heading "SampleID). I have to find a way to do this in R
meta_data_new<-read.csv("res/meta_data_paired_Placebo072.csv")
```
...
res
write_csv(res$out, "res/res_Placebo072_paired.csv")
```
...
ancom_da$taxa_id #0
write.csv(ancom_da, "res/Placebo072.csv")
...
ancom_da<-ancom_res %>% 
  filter(detected_0.6==TRUE)
ancom_da$taxa_id #0
write.csv(ancom_da, "res/Placebo_072_paired.csv")
```
